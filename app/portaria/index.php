...
<?php
    ...
    $codigo    =  $_GET['codigo'];
    $consulta = "SELECT Ingresso.codigo, Evento.id as idEvento, Evento.nome as evento, Evento.produtor, Vendedor.nome as vendedor, Cliente.nome as cliente, Ingresso.valor, Ingresso.validade, Ingresso.motivoInvalidar, Ingresso.data, Ingresso.horaLeitura, Lote.nome as lote
    FROM Ingresso 
            ...
            if($idUsuarioPortaria == ""){
                    echo'<form action="usuarioPortaria.php" id="usuarioPortaria" method="POST">';
                        echo'<div class="form-row">';
                            ...
                            echo'</div>';
                        echo'</div>';
                        echo'<div class="form-group mt-4 mb-0"><button class="btn btn-primary btn-block" type="submit" form="usuarioPortaria" >Validar produtor</button></div>';
                    echo'</form>';
            }else{
                    echo '<h6>Produtor: '. $nomeUsuarioPortaria . '</h6>';
